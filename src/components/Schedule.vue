<template>
  <div class="relative h-screen w-full bg-cover bg-center bg-fixed flex items-center justify-center" style="background-image: url('/images/pic/bg2.webp')">
    <!-- 主容器 -->
    <div class="flex flex-col md:flex-row w-full h-full pt-16 md:pt-20 pb-2 px-6 md:px-12 gap-3 md:gap-8">
        ...
        <div v-for="(tech, index) in techStack" :key="tech.name" class="flex flex-col">
          ...
          <div class="relative flex items-center h-[5vh] md:h-10 w-full">
            ...
            <div
              :ref="
                (el) => {
                  if (el) barRefs[index] = el as HTMLElement;
                }
              "
            ...
            >
              ...
              <span
                :ref="
                  (el) => {
                    if (el) textRefs[index] = el as HTMLElement;
                  }
                "
      ...
      <div class="flex-auto md:flex-1 w-full flex flex-col items-center justify-center overflow-hidden">
        <div
          class="text-center text-amber-50/90 max-w-sm md:max-w-lg w-full px-6 py-6 md:px-8 md:py-12 overflow-hidden bg-black/20 backdrop-blur-sm rounded-3xl border border-white/10 shadow-2xl"
        >
          <h2 class="text-xl md:text-3xl font-light mb-1 uppercase tracking-widest">忘れじの言の葉</h2>
          ...
          <div class="poetic-lines space-y-2 text-xs md:text-lg font-light tracking-[0.15em] leading-relaxed">
            <p>渴求着寻找着徘徊着然后被诗人传颂</p>
            <p>成了几千几万几亿的旋律</p>
            <p>即使终有一天我的命运也会被夺走后消失</p>
            <p>这也是无法忘记的故事啊</p>
          </div>
        </div>
      </div>
...
/* 斜纹图案与动画 (反向倾斜，向右滚动) */
.tech-bar-pattern {
  background-image: repeating-linear-gradient(-45deg, rgba(255, 255, 255, 0.15), rgba(255, 255, 255, 0.15) 30px, rgba(0, 0, 0, 0.05) 30px, rgba(0, 0, 0, 0.05) 60px);
